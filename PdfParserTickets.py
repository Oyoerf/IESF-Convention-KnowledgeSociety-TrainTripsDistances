...
Extrait uniquement la référence, le nom et le prénom du voyageur 1.
"""
...
def extract_travelers(text):
    """
    Extrait le nom et prénom de tous les voyageurs 1 depuis le texte du PDF.
    
    Args:
        text: Texte complet du PDF
    
    Returns:
        list: Liste de dictionnaires avec 'nom' et 'prenom' pour chaque voyageur 1 trouvé
    """
    ...
    for match in matches:
        ...
        if full_name:
            ...
            # Format attendu : "PRENOM NOM" - le premier mot est le prénom, le reste est le nom
            parts = full_name.split()
            
            if len(parts) >= 2:
                # Le premier élément est le prénom, tout le reste est le nom
                prenom = parts[0]
                nom = ' '.join(parts[1:])
                ...
                # Un seul mot, on le met comme prénom
                travelers.append({
                    'prenom': parts[0],
                    'nom': ''
                })
...
def extract_pdf_data(pdf_path):
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            # et on associe chaque référence au voyageur correspondant
            min_count = min(len(references), len(travelers))
            
            results = []
            for i in range(min_count):
                results.append({
                    'Nom': travelers[i].get('nom', ''),
                    'Prenom': travelers[i].get('prenom', ''),
                    'Ref': references[i]
                })
...
def process_batch(input_dir, output_file):
    ...
    # Définir l'ordre des colonnes : Nom, Prenom, Ref
    df = df[['Nom', 'Prenom', 'Ref']]
